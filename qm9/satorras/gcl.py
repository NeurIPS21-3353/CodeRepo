...
class GCL_linear(GCL_basic):
    ...
    def __init__(self, input_nf, output_nf, edges_in_nf=0, act_fn=None, bias=True):
        super(GCL_linear, self).__init__()
        self.act_fn = act_fn
        input_edge = input_nf * 2
        self.edge_mlp = nn.Sequential(
            nn.Linear(input_edge + edges_in_nf, output_nf, bias=bias)
        )

        #if act_fn is None:
        #    self.node_mlp = nn.Sequential(
        #        nn.Linear(hidden_nf + input_nf, output_nf, bias=bias))
        #else:
        #    self.node_mlp = nn.Sequential(
        #        nn.Linear(hidden_nf + input_nf, output_nf, bias=bias),
        #        act_fn)
    ...
    def node_model(self, x, edge_index, edge_attr):
        row, col = edge_index
        out = unsorted_segment_mean(edge_attr, row, num_segments=x.size(0))
...
class GCL_linear_basic(GCL_basic):
    """Graph Neural Net with global state and fixed number of nodes per graph.
    Args:
          hidden_dim: Number of hidden units.
          num_nodes: Maximum number of nodes (for self-attentive pooling).
    ...
    def __init__(self, input_nf, output_nf, act_fn=None, bias=True):
        super(GCL_linear, self).__init__()
        self.act_fn = act_fn
        input_edge = input_nf
        self.edge_mlp = nn.Sequential(
            nn.Linear(input_edge, output_nf, bias=bias)
        )
    ...
    def edge_model(self, source, target, edge_attr):
        out = torch.cat([self.edge_mlp(source), self.edge_mlp(target)], dim=1)
        return out

    def node_model(self, x, edge_index, edge_attr):
        row, col = edge_index
        out = unsorted_segment_sum(edge_attr, row, num_segments=x.size(0))
        if self.act_fn is not None:
            out = self.act_fn(out)
        return out
...
class GCL_SNE(GCL_basic):
    ...
    def __init__(self):
        super(GCL_SNE, self).__init__()

        #w, b, gamma = 1.0571, -9.3, 0.0468
        #w, b, gamma = 1, -10, 0.01
        w, b, gamma = 1.0468, -6.32, 0.01
        self.w = nn.Parameter(torch.ones(1)*w, requires_grad=True)
        self.b = nn.Parameter(torch.ones(1)*b, requires_grad=True)
        self.gamma = nn.Parameter(torch.ones(1)*gamma, requires_grad=True)
        self.reg = 1e-4

    def edge_model(self, source, target, edge_attr):
        x_diff = source - target
        e_pred = torch.sigmoid(torch.sum(x_diff ** 2, dim=1) * self.w + self.b).unsqueeze(1)
        m_ij = (edge_attr - e_pred) * x_diff * self.w * self.gamma * 2
        return m_ij

    def node_model(self, x, edge_index, edge_attr):
        row, col = edge_index
        agg = unsorted_segment_sum(edge_attr, row, num_segments=x.size(0))
        x_out = x + agg - self.gamma*2*x*self.reg
        return x_out
...
class E_GCL(nn.Module):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, K=None, edges_in_d=0, nodes_att_dim=0, act_fn=nn.ReLU(), recurrent=True, coords_weight=1.0, attention=False, clamp=False, dropout=[0.0, 0.0]):
